...
function CommunicationsPage({ programs, schools, templates, BACKEND, authHeaders, showToast }:{
  ...
  // Student list (loaded when user picks schools + students recipient)
  ...
  // Load smtp configs once
  useEffect(()=>{
    const headers = authHeaders() as any;
    fetch(`${BACKEND}/api/admin/broadcast/config`, { headers })
      .then(r=>r.json())
      .then(d=>{ setSmtpOptions(d.smtpConfigs??[]); setWaProvider(d.whatsappProvider??null); if(d.smtpConfigs?.[0]) setSmtpConfigId(d.smtpConfigs[0].id); })
      .catch(()=>{});
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[]);

  // Load students when school selection and recipient includes students
  async function loadStudents(schoolIds: string[]) {
    ...
    try {
      ...
      // Fetch via registrations API — we just need name/email/phone
      const res = await fetch(`${BACKEND}/api/admin/registrations?school_ids=${schoolIds.join(',')}`, { headers });
      const data = await res.json();
      const regs = (data.registrations ?? data.data ?? []) as any[];
      setAllStudents(regs);
  ...
  function toggleAllStudents(){ filteredStudents.length>0&&selectedStudents.size===filteredStudents.length ? setSelectedStudents(new Set()) : setSelectedStudents(new Set(filteredStudents.map(s=>s.id))); }
  ...
  function reset(){ setProgramId(''); setSelectedSchools(new Set()); setRecipients(new Set(['schools'])); setChannel(''); setTemplateId(''); setResult(null); setStep(1); setSchoolSearch(''); setStudentSearch(''); setAllStudents([]); setSelectedStudents(new Set()); if(smtpOptions[0]) setSmtpConfigId(smtpOptions[0].id); }
  ...
  async function handleSend(){
    ...
    try {
      ...
      const res = await fetch(`${BACKEND}/api/admin/broadcast`, { method:'POST', headers, body: JSON.stringify(body) });
      const data = await res.json();
  ...
  const chipSel = (active:boolean): React.CSSProperties => ({
    padding:'9px 18px', borderRadius:9, cursor:'pointer', fontSize:13, fontWeight:700, userSelect:'none' as const,
    border:`1.5px solid ${active?'var(--acc)':'var(--bd)'}`,
    background:active?'var(--acc3)':'transparent',
    color:active?'var(--acc)':'var(--m)', transition:'all .12s',
  });
  ...
  const stepLabels = [
    {n:1,label:'Program'},{n:2,label:'Schools'},{n:3,label:'Recipients'},
    {n:4,label:'Channel & Template'},{n:5,label:'Results'},
  ] as const;
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:0,marginBottom:24,overflowX:'auto',paddingBottom:4}}>
        {stepLabels.map((s,i)=>(
          <React.Fragment key={s.n}>
            <div style={{display:'flex',alignItems:'center',gap:7,cursor:s.n<step?'pointer':'default',opacity:s.n>step?0.45:1}}
              onClick={()=>{ if(s.n<step) setStep(s.n as any); }}>
              <div style={stepDot(step===s.n,step>s.n)}>{step>s.n?'✓':s.n}</div>
              <span style={{fontSize:12,fontWeight:step===s.n?700:500,color:step===s.n?'var(--acc)':step>s.n?'#10b981':'var(--m)',whiteSpace:'nowrap'}}>{s.label}</span>
            </div>
            {i<stepLabels.length-1&&<div style={{flex:1,height:2,minWidth:16,background:step>s.n?'#10b981':'var(--bd)',margin:'0 6px'}}/>}
          </React.Fragment>
        ))}
      </div>
      ...
      {step===1&&(
        <div style={card}>
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:10}}>
            <div onClick={()=>setProgramId('')} style={chipSel(programId==='')}>🌐 All Programs</div>
            {programs.filter(p=>p.status==='active').map(p=>(
              <div key={p.id} onClick={()=>setProgramId(p.id)} style={chipSel(programId===p.id)}>🎯 {p.name}</div>
            ))}
          </div>
      ...
      {/* ═══ STEP 3: Recipients + Student List ════════════════════ */}
      {step===3&&(
        <div style={card}>
          <div style={{fontWeight:800,fontSize:16,marginBottom:4}}>👥 Who to Send To?</div>
          <div style={{fontSize:12,color:'var(--m)',marginBottom:16}}>Choose one or both. When "Students" is selected you can fine-tune the list below.</div>

          {/* Recipient toggle cards */}
          <div style={{display:'flex',gap:12,flexWrap:'wrap',marginBottom:20}}>
            {([
              {id:'schools'  as const,icon:'🏫',label:'School Contacts',  desc:'Primary contact of each school'},
              ...
              return(
                <div key={r.id} onClick={()=>toggleRecipient(r.id)} style={{flex:1,minWidth:200,padding:'16px 18px',borderRadius:12,cursor:'pointer',border:`2px solid ${active?'var(--acc)':'var(--bd)'}`,background:active?'var(--acc3)':'var(--bg)',transition:'all .12s'}}>
                  <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:5}}>
                    <span style={{fontSize:20}}>{r.icon}</span>
                    <span style={{fontWeight:800,fontSize:14,color:active?'var(--acc)':'var(--text)'}}>{r.label}</span>
                    {active&&<span style={{marginLeft:'auto',fontSize:15}}>✅</span>}
                  </div>
          ...
          {/* ── Student list (when students selected) ── */}
          {recipients.has('students')&&(
            <div style={{border:'1.5px solid var(--bd)',borderRadius:12,overflow:'hidden',marginBottom:16}}>
              {/* Header */}
              <div style={{padding:'12px 16px',background:'var(--bg)',borderBottom:'1px solid var(--bd)',display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
                <span style={{fontWeight:700,fontSize:13}}>👨‍🎓 Student List</span>
                <span style={{fontSize:11,color:'var(--m)'}}>Fetched from {selectedSchools.size} school(s) · All selected by default</span>
                <div style={{marginLeft:'auto',display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                  ...
                  <div style={{position:'relative'}}>
                    ...
                    <input value={studentSearch} onChange={e=>setStudentSearch(e.target.value)} placeholder="Search students…"
                      style={{padding:'5px 10px 5px 26px',borderRadius:7,border:'1.5px solid var(--bd)',fontSize:12,background:'var(--card)',color:'var(--text)',outline:'none',width:160}}/>
                  </div>
                  {/* Select / Remove all */}
                  <button onClick={()=>setSelectedStudents(new Set(filteredStudents.map(s=>s.id)))}
                    style={{padding:'5px 12px',borderRadius:7,border:'1.5px solid #10b981',background:'rgba(16,185,129,0.08)',color:'#10b981',fontSize:11,fontWeight:700,cursor:'pointer'}}>
                    ✅ Select All
                  </button>
                  <button onClick={()=>setSelectedStudents(new Set())}
                    style={{padding:'5px 12px',borderRadius:7,border:'1.5px solid #ef4444',background:'rgba(239,68,68,0.08)',color:'#ef4444',fontSize:11,fontWeight:700,cursor:'pointer'}}>
                    🚫 Remove All
                  </button>
                  <span style={{fontSize:11,fontWeight:700,color:'var(--acc)'}}>{selectedStudents.size}/{allStudents.length} selected</span>
                </div>
              </div>
                  ...
                  :<div style={{maxHeight:320,overflowY:'auto'}}>
                    ...
                    <div onClick={toggleAllStudents} style={{display:'flex',alignItems:'center',gap:12,padding:'8px 16px',background:'var(--acc3)',cursor:'pointer',borderBottom:'1px solid var(--bd)'}}>
                      ...
                      <span style={{fontSize:12,fontWeight:700,color:'var(--acc)'}}>
                        {selectedStudents.size===filteredStudents.length?'Deselect All on this page':`Select All on this page`}
                      </span>
                    </div>
                      ...
                      return(
                        <div key={s.id} onClick={()=>toggleStudent(s.id)} style={{display:'flex',alignItems:'center',gap:12,padding:'9px 16px',cursor:'pointer',borderBottom:'1px solid var(--bd)',background:isSel?'rgba(79,70,229,0.03)':'transparent',transition:'background .1s'}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{fontSize:11,color:'var(--m)'}}>{s.class_grade?`Grade ${s.class_grade} · `:''}{school?.name||''}</div>
                          </div>
                          ...
                          {!isSel&&<span style={{fontSize:10,padding:'2px 7px',borderRadius:20,background:'rgba(239,68,68,0.1)',color:'#ef4444',fontWeight:700,flexShrink:0}}>Excluded</span>}
                        </div>
                      );
                    })}
                  </div>
          ...
          <div style={{padding:'10px 14px',background:'var(--bg)',borderRadius:8,border:'1px solid var(--bd)',fontSize:12,color:'var(--m)',marginBottom:16}}>
            ...
            {recipients.has('students')&&!loadingStudents&&allStudents.length>0&&<> · <strong>{selectedStudents.size}</strong> students selected</>}
          </div>
      ...
      {/* ═══ STEP 4: Channel, Email Sender & Template ════════════════ */}
      {step===4&&(
        <div style={card}>
          <div style={{fontWeight:800,fontSize:16,marginBottom:4}}>✉️ Channel &amp; Template</div>
          <div style={{fontSize:12,color:'var(--m)',marginBottom:18}}>Choose channel, configure sender, and pick a template.</div>
          ...
          <div style={{marginBottom:20}}>
            ...
            <div style={{display:'flex',gap:12}}>
              {([{id:'email' as const,icon:'✉️',label:'Email'},{id:'whatsapp' as const,icon:'💬',label:'WhatsApp'}]).map(c=>(
                <div key={c.id} onClick={()=>{setChannel(c.id);setTemplateId('');}} style={{flex:1,padding:'14px 18px',borderRadius:10,cursor:'pointer',border:`2px solid ${channel===c.id?'var(--acc)':'var(--bd)'}`,background:channel===c.id?'var(--acc3)':'var(--bg)',display:'flex',alignItems:'center',gap:10,transition:'all .12s'}}>
          ...
          {channel==='email'&&smtpOptions.length>0&&(
            <div style={{marginBottom:20,padding:'14px 16px',borderRadius:12,background:'rgba(79,70,229,0.05)',border:'1.5px solid rgba(79,70,229,0.2)'}}>
              <div style={{fontSize:11,fontWeight:700,color:'var(--acc)',textTransform:'uppercase' as const,letterSpacing:'.05em',marginBottom:10}}>📤 Send From (Email Account)</div>
              <div style={{display:'flex',flexDirection:'column' as const,gap:8}}>
                {smtpOptions.map(opt=>{
                  const active=smtpConfigId===opt.id;
                  const prog=opt.program_id?programs.find(p=>p.id===opt.program_id):null;
                  return(
                    <div key={opt.id} onClick={()=>setSmtpConfigId(opt.id)} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 14px',borderRadius:9,cursor:'pointer',border:`2px solid ${active?'var(--acc)':'var(--bd)'}`,background:active?'var(--acc3)':'var(--bg)',transition:'all .12s'}}>
                      <div style={stepDot(active,false)}>{active?'✓':'@'}</div>
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:11,color:'var(--m)',marginTop:2}}>
                          ...
                          {prog&&<span style={{marginLeft:8,padding:'1px 7px',borderRadius:20,background:'rgba(139,92,246,0.1)',color:'#8b5cf6',fontWeight:700}}>🎯 {prog.name}</span>}
                          {!opt.program_id&&<span style={{marginLeft:8,padding:'1px 7px',borderRadius:20,background:'var(--bg)',color:'var(--m)',border:'1px solid var(--bd)'}}>🌐 Global</span>}
                        </div>
                      </div>
                      {active&&<span style={{fontSize:14,color:'var(--acc)'}}>✅</span>}
                    </div>
                  );
                })}
              </div>
              {smtpOptions.length===0&&<div style={{fontSize:12,color:'#ef4444'}}>No SMTP configs found. Configure in Integrations first.</div>}
            </div>
          )}
          ...
          {channel==='whatsapp'&&(
            <div style={{marginBottom:20,padding:'12px 14px',borderRadius:10,background:'rgba(37,211,102,0.06)',border:'1.5px solid rgba(37,211,102,0.25)'}}>
              <div style={{fontSize:12,color:'var(--text)'}}>
                💬 <strong>WhatsApp Provider:</strong> {waProvider?waProvider.label:<span style={{color:'#ef4444'}}>Not configured — set up in Integrations</span>}
              </div>
            </div>
          )}
          ...
          {channel&&(
            <div style={{marginBottom:20}}>
              <div style={{fontSize:11,fontWeight:700,color:'var(--m)',textTransform:'uppercase' as const,letterSpacing:'.05em',marginBottom:10}}>
                Template
                <span style={{fontWeight:400,textTransform:'none' as const,marginLeft:6}}>{filteredTemplates.length} active {channel} template{filteredTemplates.length!==1?'s':''}</span>
              </div>
              {filteredTemplates.length===0
                ?<div style={{padding:'14px',borderRadius:8,background:'rgba(239,68,68,0.06)',border:'1px solid rgba(239,68,68,0.2)',fontSize:13,color:'#ef4444'}}>No active {channel} templates. Create one in Message Templates.</div>
                :<div style={{display:'flex',flexDirection:'column' as const,gap:8}}>
                  {filteredTemplates.map(t=>{
                    const active=templateId===t.id;
                    return(
                      <div key={t.id} onClick={()=>setTemplateId(t.id)} style={{padding:'13px 15px',borderRadius:10,cursor:'pointer',border:`2px solid ${active?'var(--acc)':'var(--bd)'}`,background:active?'var(--acc3)':'var(--bg)',transition:'all .12s'}}>
          ...
          {channel&&templateId&&(
            <div style={{padding:'14px 16px',borderRadius:10,background:'rgba(16,185,129,0.06)',border:'1.5px solid rgba(16,185,129,0.25)',marginBottom:16}}>
              ...
              <div style={{fontSize:12,lineHeight:2}}>
                ...
                <div>👥 Recipients: <strong>{Array.from(recipients).map(r=>r==='schools'?'School Contacts':'Students').join(' + ')}</strong></div>
                {recipients.has('students')&&<div>👨‍🎓 Students: <strong>{selectedStudents.size}</strong> selected</div>}
          ...
          <div style={{display:'flex',justifyContent:'space-between',marginTop:8}}>
            ...
            <button className="btn btn-primary" disabled={!channel||!templateId||sending} style={{minWidth:150}}
              onClick={handleSend}>
              {sending?<span>⏳ Sending…</span>:'📢 Send Broadcast'}
            </button>
          </div>
        </div>
      )}
