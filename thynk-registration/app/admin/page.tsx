                ...
                {consultants.map((c,i)=>{
                  ...
                  return (
                    <tr key={c.id} style={{borderBottom:'1px solid var(--bd)',background:i%2===0?'transparent':'rgba(0,0,0,.015)'}}>
                      ...
                      <td style={{padding:'10px 14px',textAlign:'center'}}>{c.is_default_consultant?<span title="Default Consultant" style={{color:'#f59e0b',fontWeight:800}}>⭐</span>:'—'}</td>
                          ...
                          <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                            ...
                            {c.consultant_code && (
                              <button className="btn" style={{fontSize:11,padding:'4px 10px',color:'#4f46e5',borderColor:'rgba(79,70,229,.3)'}}
                                onClick={()=>{
                                  // Copy curated link — needs a program slug; copy base pattern
                                  navigator.clipboard.writeText(`${BACKEND||'https://thynk-registration.vercel.app'}/registration/${c.consultant_code}/`);
                                  showToast('Curated link base copied — append program slug!','🔗');
                                }}>
                                🔗 Copy Link
                              </button>
                            )}
...
function ConsultantFormModal({ initial, BACKEND, authHeaders, onClose, onSave, showToast }: {
  ...
  const [consultantCode,   setConsultantCode]  = React.useState(initial.consultant_code  ?? '');
  const [mobileNumber,     setMobileNumber]    = React.useState(initial.mobile_number    ?? '');
  const [panNumber,        setPanNumber]       = React.useState(initial.pan_number       ?? '');
  ...
  async function handleSave() {
    ...
    if (!consultantCode.trim()) { setError('Consultant Code is required'); return; }
    if (!/^[a-z0-9-]+$/.test(consultantCode.trim().toLowerCase())) {
      setError('Consultant Code must be lowercase letters, digits, or hyphens only (e.g. cons001)'); return;
    }
    setSaving(true); setError('');
    try {
      const method = isEdit ? 'PATCH' : 'POST';
      const body = isEdit
        ? { id:initial.id, name, ...(password?{password}:{}), consultant_code:consultantCode.trim().toLowerCase(), mobile_number:mobileNumber.trim()||null, pan_number:panNumber.trim()||null, is_default_consultant:isDefault }
        : { name, email, password, consultant_code:consultantCode.trim().toLowerCase(), mobile_number:mobileNumber.trim()||null, pan_number:panNumber.trim()||null, is_default_consultant:isDefault };
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}
      ...
      <div style={{background:'var(--card)',border:'1.5px solid var(--bd)',borderRadius:20,width:'100%',maxWidth:500,padding:'28px 28px 24px',maxHeight:'90vh',overflowY:'auto'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 14px'}}>
          ...
          <div style={{marginBottom:14}}>
            <label style={CFM_LABEL}>Consultant Code *</label>
            <input style={CFM_IS} value={consultantCode} onChange={e=>setConsultantCode(e.target.value.toLowerCase())} placeholder="cons001" />
            <div style={{fontSize:10,color:'var(--m)',marginTop:3}}>Used in curated link: /registration/<strong>{consultantCode||'code'}</strong>/program</div>
          </div>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 14px'}}>
          <div style={{marginBottom:14}}>
            <label style={CFM_LABEL}>Mobile Number</label>
            <input style={CFM_IS} value={mobileNumber} onChange={e=>setMobileNumber(e.target.value)} placeholder="+91 98765 43210" type="tel" />
          </div>
          <div style={{marginBottom:14}}>
            <label style={CFM_LABEL}>PAN Number</label>
            <input style={{...CFM_IS,textTransform:'uppercase'}} value={panNumber} onChange={e=>setPanNumber(e.target.value.toUpperCase())} placeholder="ABCDE1234F" />
          </div>
        </div>

        {/* Default Consultant toggle */}
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16,background:'rgba(79,70,229,.06)',border:'1px solid rgba(79,70,229,.2)',borderRadius:10,padding:'10px 14px'}}>
          <input type="checkbox" id="is_default_consultant" checked={isDefault} onChange={e=>setIsDefault(e.target.checked)} style={{width:16,height:16,cursor:'pointer'}} />
          <label htmlFor="is_default_consultant" style={{fontSize:13,fontWeight:700,color:'var(--text)',cursor:'pointer'}}>
            Set as Default Consultant
          </label>
          <span style={{fontSize:11,color:'var(--m)',flex:1}}>Schools registered via the generic link (/registration/program) will be tagged to this consultant.</span>
        </div>
        ...
        {!isEdit && (
          <div style={{background:'rgba(79,70,229,.06)',border:'1px solid rgba(79,70,229,.2)',borderRadius:10,padding:'10px 14px',fontSize:12,color:'var(--m)',marginBottom:14}}>
            💡 After creation, share the <strong>Consultant Portal URL</strong> along with these credentials.<br/>
            Curated link will be: <code style={{fontSize:11}}>/registration/{consultantCode||'<code>'}/&lt;program&gt;</code>
          </div>
        )}
        <div style={{display:'flex',gap:10,justifyContent:'flex-end',marginTop:8}}>
          <button className="btn btn-outline" onClick={onClose}>Cancel</button>
