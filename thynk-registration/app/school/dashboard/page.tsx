...
export default function SchoolDashboard() {
  ...
  useChartJs(() => {
    ...
    if (ctx) {
      ...
      gradTotal.addColorStop(0, 'rgba(99,102,241,0.18)');
      gradTotal.addColorStop(1, 'rgba(99,102,241,0)');
      const gradPaid = ctx.createLinearGradient(0, 0, 0, 220);
      gradPaid.addColorStop(0, 'rgba(16,185,129,0.22)');
      gradPaid.addColorStop(1, 'rgba(16,185,129,0)');
      chartsRef.current.daily = new C(ctx, {
        ...
        options: {
          ...
          plugins: {
            legend: { position: 'top', labels: { font: { size: 11, family: 'DM Sans, sans-serif' }, boxWidth: 10, usePointStyle: true, pointStyle: 'circle' } },
    ...
    if (ctx2) chartsRef.current.status = new C(ctx2, {
      ...
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: {
          legend: { position: 'bottom', labels: { font: { size: 11, family: 'DM Sans, sans-serif' }, boxWidth: 10, usePointStyle: true, pointStyle: 'circle', padding: 14 } },
    ...
    if (ctx3 && sortedClasses.length > 0) chartsRef.current.classBar = new C(ctx3, {
      ...
      data: {
        ...
        datasets: [{
          label: 'Paid Students',
          data: sortedClasses.map(c => byClass[c]),
    ...
    style.textContent = `
      ...
      * { box-sizing: border-box; }
      ...
      .stat-card {
        ...
        position: relative;
        overflow: hidden;
        animation: floatUp .5s cubic-bezier(0.34,1.56,0.64,1) both;
        cursor: default;
      ...
      <div style={{ minHeight: '100vh', background: '#f0f2f7', fontFamily: 'DM Sans, sans-serif', color: '#0f172a' }}>
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button onClick={load}
              style={{ background: '#f1f5f9', color: '#475569', border: 'none', borderRadius: 10, padding: '7px 15px', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all .15s', display: 'flex', alignItems: 'center', gap: 6 }}
            ...
            <button onClick={doLogout}
              style={{ background: '#fff0f0', color: '#dc2626', border: 'none', borderRadius: 10, padding: '7px 15px', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all .15s' }}
        ...
        <div style={{ display: 'flex', minHeight: 'calc(100vh - 64px)' }}>
          ...
          {/* Sidebar */}
          <aside style={{ width: 220, background: '#ffffff', borderRight: '1px solid #e2e8f0', padding: '20px 12px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 0 }}>
            {TABS.map((t, idx) => (
              <button
                key={t.id}
            ...
            {/* Quick stats panel */}
            <div style={{ marginTop: 24, background: 'linear-gradient(135deg,#f8fafc,#f1f5f9)', borderRadius: 14, padding: '16px 14px', border: '1px solid #e2e8f0', animation: 'floatUp .5s ease both', animationDelay: '200ms' }}>
            ...
            {/* Conversion radial at bottom of sidebar */}
            <div style={{ marginTop: 'auto', paddingTop: 20, display: 'flex', justifyContent: 'center', animation: 'floatUp .6s ease both', animationDelay: '350ms' }}>
              <RadialProgress
                value={stats?.paid ?? 0}
                max={stats?.total ?? 1}
                color="#6366f1"
                size={110}
                label="Conversion"
                sublabel={`${stats?.paid ?? 0} of ${stats?.total ?? 0}`}
              />
            </div>
          </aside>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 26 }}>
                  <div style={{ animation: 'floatUp .3s ease both' }}>
                    ...
                    <p style={{ margin: '3px 0 0', fontSize: 12, color: '#94a3b8', fontWeight: 500 }}>
                      {school?.name} · Last updated just now
                    </p>
                  </div>
                  <div style={{
                    background: convPct >= 60 ? 'linear-gradient(135deg,#d1fae5,#a7f3d0)' : 'linear-gradient(135deg,#fef3c7,#fde68a)',
                    color: convPct >= 60 ? '#065f46' : '#92400e',
                    padding: '8px 16px',
                    borderRadius: 24,
                    fontSize: 13,
                    fontWeight: 800,
                    animation: 'scaleIn .4s ease both 100ms',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 18, marginBottom: 18 }}>
                  <div className="glass-card" style={{ animationDelay: '100ms' }}>
                    <div className="card-header">
                      <span className="card-header-icon">📅</span>
                      <span className="card-title">Daily Registrations — Last 30 Days</span>
                    </div>
                ...
                {/* Class bar chart + revenue timeline */}
                <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 18, marginBottom: 18 }}>
                  <div className="glass-card" style={{ animationDelay: '200ms' }}>
                    <div className="card-header">
                      <span className="card-header-icon">🏆</span>
                      <span className="card-title">Top Classes by Paid Count</span>
                    </div>
              ...
              <div>
                  ...
                  {([
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 9, transition: 'all .18s', fontFamily: 'DM Sans, sans-serif',
                        background: studentTab === st.id ? '#fff' : 'transparent',
                        color: studentTab === st.id ? st.color : '#64748b',
                        boxShadow: studentTab === st.id ? '0 1px 6px rgba(0,0,0,0.08)' : 'none',
                        display: 'flex', alignItems: 'center', gap: 6,
                      }}
                    >
                      <span style={{ fontSize: 13 }}>{st.icon}</span>
                      {st.label}
                      <span style={{ background: studentTab === st.id ? `${st.color}15` : 'transparent', color: st.color, borderRadius: 20, fontSize: 11, padding: '1px 7px', fontWeight: 800, transition: 'all .18s' }}>
                        {st.count}
                      </span>
                    </button>
                  ))}
                ...
                <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', animation: 'floatUp .35s ease both 100ms' }}>
                  <input
                    ...
                    style={{ flex: 1, minWidth: 220, border: '1.5px solid #e2e8f0', borderRadius: 11, padding: '9px 14px', fontSize: 13, fontFamily: 'DM Sans, sans-serif', outline: 'none', color: '#0f172a', background: '#fff', transition: 'border-color .15s, box-shadow .15s' }}
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', fontSize: 12, color: '#94a3b8', padding: '0 4px', fontWeight: 500 }}>
                  ...
                  <button
                    ...
                    style={{ background: 'linear-gradient(135deg,#10b981,#059669)', color: '#fff', border: 'none', borderRadius: 11, padding: '9px 18px', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap', boxShadow: '0 2px 10px rgba(16,185,129,0.35)', transition: 'opacity .15s', fontFamily: 'DM Sans, sans-serif' }}
