// app/api/school/list/route.ts
// PUBLIC endpoint — no auth required
// Returns schools that are approved + active for registration, filtered by project/city/state

import { NextRequest, NextResponse } from 'next/server';
import { createServiceClient } from '@/lib/supabase/server';

export const revalidate = 60; // cache 60s

export async function GET(req: NextRequest) {
  const { searchParams } = new URL(req.url);
  const projectSlug = searchParams.get('project') || '';
  const city        = (searchParams.get('city')  || '').trim().toLowerCase();
  const state       = (searchParams.get('state') || '').trim().toLowerCase();

  if (!projectSlug) {
    return NextResponse.json({ error: 'project param required' }, { status: 400 });
  }

  const supabase = createServiceClient();

  let query = supabase
    .from('schools')
    .select('id, school_code, name, city, state, country, pricing(id, program_name, base_amount, currency, gateway_sequence, is_active)')
    .eq('project_slug', projectSlug)
    .eq('status', 'approved')
    .eq('is_active', true)
    .eq('is_registration_active', true);

  // Filter by city/state if provided (case-insensitive via ilike)
  if (city)  query = query.ilike('city',  city);
  if (state) query = query.ilike('state', state);

  const { data, error } = await query.order('name', { ascending: true });

  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }

  // Only return schools that have at least one active pricing entry
  const now = new Date().toISOString();
  const schools = (data ?? []).filter(s =>
    (s.pricing as any[]).some((p: any) => p.is_active)
  ).map(s => ({
    ...s,
    pricing: (s.pricing as any[]).filter((p: any) => p.is_active),
  }));

  return NextResponse.json({ schools });
}
