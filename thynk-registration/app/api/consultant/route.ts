...
// Single endpoint for consultants:
//   GET  ?view=schools  → their schools list
//   GET  ?view=report   → aggregated report across all their schools
//   GET  ?view=report&school_id=X → single-school detailed report
...
export async function GET(req: NextRequest) {
  ...
  const view = searchParams.get('view') ?? 'schools';
  const schoolIdFilter = searchParams.get('school_id');

  // Get all schools belonging to this consultant
  const { data: mySchools, error: schoolErr } = await service
    .from('schools')
    .select(`
      id, school_code, name, org_name, city, state, country,
  ...
  if (!mySchools?.length) {
    return NextResponse.json({ schools: [], stats: null, rows: [] });
  }

  if (view === 'schools') {
    return NextResponse.json({ schools: mySchools });
  }

  // ── Report view ───────────────────────────────────────────────────────────
  const schoolIds = schoolIdFilter
    ? [schoolIdFilter]
    : mySchools.map(s => s.id);

  const { data: rows, error: regErr } = await service
    .from('registrations')
    .select(`
      id, created_at, school_id, student_name, class_grade, gender,
  ...
  // Per-school summary
  const bySchool: Record<string, { name: string; total: number; paid: number; revenue: number }> = {};
  for (const s of mySchools) {
    bySchool[s.id] = { name: s.name, total: 0, paid: 0, revenue: 0 };
  }
  for (const r of deduped) {
    if (r.school_id && bySchool[r.school_id]) {
      bySchool[r.school_id].total++;
      if (r.payment_status === 'paid') {
        bySchool[r.school_id].paid++;
        bySchool[r.school_id].revenue += r.final_amount ?? 0;
      }
    }
  }
