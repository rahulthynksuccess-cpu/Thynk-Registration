...
async function fetchSchool(schoolCode: string) {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'https://www.thynksuccess.com';
  const res = await fetch(`${baseUrl}/api/school/${schoolCode}`, {
    next: { revalidate: 300 },
  });
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const school = await fetchSchool(params.schoolCode);
