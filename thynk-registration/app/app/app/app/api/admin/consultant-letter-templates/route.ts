// app/api/admin/consultant-letter-templates/route.ts
// PDF letter templates, one per program, for the CONSULTANT letter generator.
// Mirrors app/api/admin/letter-templates/route.ts (schools) but uses
// consultant_name_token / consultant_code_token and the
// "consultant-letter-templates" storage bucket.
...
// ── GET /api/admin/consultant-letter-templates?projectId=xxx ─────────────────
export async function GET(req: NextRequest) {
  ...
  const templates = await Promise.all((data ?? []).map(async (t: any) => {
    const { data: signed } = await service.storage
      .from('consultant-letter-templates')
...
// ── POST /api/admin/consultant-letter-templates (multipart/form-data) ────────
export async function POST(req: NextRequest) {
  ...
  const consultantNameToken = (formData.get('consultantNameToken') as string) || 'Consultant Name';
  const consultantCodeToken = (formData.get('consultantCodeToken') as string) || 'tscons000';
  ...
  const { data: existing } = await service
    .from('consultant_letter_templates').select('id, file_path').eq('project_id', projectId).single();

  if (existing) {
    await service.storage.from('consultant-letter-templates').remove([existing.file_path]);
    const { data: updated, error: ue } = await service
      .from('consultant_letter_templates')
      .update({
        file_name: file.name, file_path: filePath, file_size: file.size,
        file_type: 'application/pdf',
        consultant_name_token: consultantNameToken, consultant_code_token: consultantCodeToken,
...
// ── DELETE /api/admin/consultant-letter-templates?id=xxx ─────────────────────
export async function DELETE(req: NextRequest) {
  ...
  const { data: tmpl } = await service
    .from('consultant_letter_templates').select('file_path').eq('id', id).single();
  if (tmpl?.file_path)
    await service.storage.from('consultant-letter-templates').remove([tmpl.file_path]);

  const { error } = await service.from('consultant_letter_templates').delete().eq('id', id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
