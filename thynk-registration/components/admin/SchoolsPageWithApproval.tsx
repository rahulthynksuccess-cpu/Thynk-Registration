        ...
        return (
          <div style={{ background:'var(--card)', border:'1.5px solid var(--bd)', borderRadius:16, padding:'18px 20px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:10 }}>
              {activeSchools.map(s => {
                const regUrl  = `https://thynksuccess.com/registration/${s.project_slug ?? ''}/?school=${s.school_code}`;
                return (
          ...
          <tbody>
            {filtered.length === 0
              ...
              : filtered.map(s => {
                  ...
                  const regUrl = `https://thynksuccess.com/registration/${s.project_slug ?? prog?.slug ?? ''}/?school=${s.school_code}`;
